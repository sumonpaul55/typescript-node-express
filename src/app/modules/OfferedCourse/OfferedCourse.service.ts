...
import { SemisterRegistration } from "../semisterRegistration/semisterRagistration.model";
...
const createOfferedCourseIntoDB = async (payload: TOfferedCourse) => {
  ...
  const isSemesterRegistrationExits = await SemisterRegistration.findById(semesterRegistration);
...
const updateOfferedCourseIntoDB = async (id: string, payload: Pick<TOfferedCourse, "faculty" | "days" | "startTime" | "endTime">) => {
  ...
  const semesterRegistrationStatus = await SemisterRegistration.findById(semesterRegistration);
...
const deleteOfferedCourseFromDB = async (id: string) => {
  ...
  const semesterRegistrationStatus = await SemisterRegistration.findById(semesterRegistation).select("status");

  if (semesterRegistrationStatus?.status !== "UPCOMING") {
    throw new AppError(httpStatus.BAD_REQUEST, `Offered course can not update ! because the semester ${semesterRegistrationStatus}`);
  }
